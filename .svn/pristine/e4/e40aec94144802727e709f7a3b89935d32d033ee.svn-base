...
// TODO CS349
    ...
    if(this->IsVisible()){
      ...
      g->SetForegroundColor(0x000000);  //window background color (default black)
      g->FillRect(this->GetBounds());

      g->SetForegroundColor(0xFFFFFF);  //graphic color (default white)
      g->SetBackgroundColor(0x000000);  //color black

      this->PaintComponent(g);      
      for(vector<Component*>::iterator iter=this->children.begin(); iter!=this->children.end(); ++iter){
        Component*c = (*iter);
        AffineTransform newTranslationTransform = AffineTransform::MakeTranslation(c->GetBounds().x, c->GetBounds().y);
        g->SetTransform(newTranslationTransform);//AffineTransform::MakeTranslation(c->GetBounds().x, c->GetBounds().y));//newTranslationTransform);
        c->Paint(g);
      }
