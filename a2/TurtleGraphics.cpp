...
namespace cs349 {
  ...
  TurtleGraphics::TurtleGraphics(const string & name, EventQueue* eventQueue)
    : SchemeComponent(name)
  {
    this->drawStepNum = 0;
    this->timer = new Timer(eventQueue, 100, false, this);
    timer->Start();
  }
  ...
  void TurtleGraphics::PaintComponent(Graphics* graphics) {
    ...
    graphics->DrawRect(Rectangle(-10, -10, 320, 320));
  ...
  void TurtleGraphics::HandleTimerEvent(const TimerEvent & e) {
    if (timer->IsRunning()) {
      Component::Repaint();
    }
  }
